...
for p in resume_data.get('projects', []):
    ...
    project_lines.append('- ' + name + ': ' + tech + ' - ' + desc)

projects_text = chr(10).join(project_lines)
skills_text = ', '.join(resume_data.get('skills', []))

prompt = 'Compare resume claims against interview answers.' + chr(10) + chr(10)
prompt += 'RESUME PROJECTS:' + chr(10) + projects_text + chr(10)
prompt += 'SKILLS: ' + skills_text + chr(10) + chr(10)
prompt += 'INTERVIEW TRANSCRIPT:' + chr(10) + qa_pairs[:3000] + chr(10) + chr(10)
prompt += 'Return ONLY JSON: {\"consistency_score\": 0, \"summary\": \"...\", \"gaps\": [\"...\"]}'
