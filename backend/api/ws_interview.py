...
async def interview_ws(
  ...
  logger.info(f"[WS] interview_ws connected interview_id={interview_id}")
  ...
  try:
    ...
    logger.info(f"[WS] disconnect interview_id={interview_id}")
...
async def handle_on_connect(
    ...
    try:
        await ws.send_json(payload)
        logger.info("[WS] sent greeting text")
    except Exception as e:
        logger.exception("[WS] failed to send greeting: %s", e)
        ...
        try:
            ...
            if audio_bytes:
                ...
                try:
                    ...
                    logger.info("[WS] sent greeting audio_url=%s", audio_path)
                except Exception as e:
                    logger.exception("[WS] failed to send greeting audio: %s", e)
        except Exception as e:
    ...
    if first_q:
        ...
        try:
            ...
            logger.exception("[WS] failed to send no-questions message: %s", e)
...
async def send_agent_question(
    ...
    try:
        await ws.send_json(payload)
        logger.info("[WS] send_json succeeded for qid=%s", question["id"])
    except Exception as e:
        logger.exception("[WS] send_json failed for qid=%s: %s", question["id"], e)
    ...
    async def gen_and_attach_audio():
        try:
            ...
            try:
                ...
                logger.exception("[WS] failed to send question audio for qid=%s: %s", question["id"], e)
        except Exception as e:
