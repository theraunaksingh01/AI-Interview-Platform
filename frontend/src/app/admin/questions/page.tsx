...
export default function AdminQuestionsPage() {
    ...
    const fetchQuestions = useCallback(async () => {
        ...
        setLoading(false);
    }, [page, search, category, difficulty, isActive, user, authLoading]);
                ...
                <div className="flex gap-3 flex-wrap">
                    {data.category_breakdown.map((c: any) => (
                        <div key={c.type} className="rounded-xl border border-[#E5E7EB] bg-white px-4 py-2.5">
                            <span className="text-[13px] font-black text-[#111]">{c.count}</span>
                    ...
                    <tbody>
                        {loading ? [...Array(10)].map((_, i) => (
                            ...
                            <tr key={q.id} className="border-b border-[#F9FAFB] hover:bg-[#F9FAFB] transition">
                                ...
                                <td className="px-4 py-3">
                                    <span className={`rounded-full px-2 py-0.5 text-[9px] font-black uppercase ${q.times_used > 0 ? "bg-emerald-100 text-emerald-700" : "bg-[#F3F4F6] text-[#9CA3AF]"}`}>
                                        {q.times_used > 0 ? `${q.times_used}x` : "unused"}
                                    </span>
                                </td>
                                <td className="px-4 py-3">
