...
>(({ title, description, logos, speed = "normal", className, ...props }, ref) => {
  const durationMap = { normal: "40s", slow: "80s", fast: "20s" };
  ...
  return (
    <>
      ...
      <section
        ref={ref}
        aria-label={title}
        className={cn("full-bleed bg-background text-foreground py-12 md:py-16", className)}
        {...props}
      >
        {/* Safely centered wide container */}
          ...
          <div className="rounded-2xl border bg-background/60 backdrop-blur-sm overflow-hidden shadow-sm">
            ...
            <div className="p-6 md:p-8 lg:p-10 border-b">
              <div className="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-6 lg:gap-8">
                ...
                <p className="text-muted-foreground self-center lg:justify-self-end text-balance">
                  {description}
                </p>
              </div>
            </div>
